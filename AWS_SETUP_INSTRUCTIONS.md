# AWS SES + Lambda Setup Instructions for PrintSmith Contact Form
...
4. Function name: `printsmith-contact-form`
...
4. Update the `region` in the code if needed (line 2):
   ```javascript
   const ses = new AWS.SES({ region: 'eu-west-1' }); // Change to your region
   ```
...
4. Choose "New API"
5. API name: `printsmith-contact-api`
6. Description: "Contact form API for PrintSmith website"
...
7. Lambda Function: `printsmith-contact-form`
8. Click "Save"
...
2. Replace `YOUR_API_GATEWAY_ENDPOINT_HERE` with your API endpoint:
   ```javascript
   const API_ENDPOINT = 'https://xxxxxxxxxx.execute-api.eu-west-1.amazonaws.com/prod/contact';
   ```
...
- Make sure you verified `contact@printsmith.ie` in SES
